...
export function AllureReportPanel({ title = "Allure report" }: { title?: string }) {
  ...
  async function buildReport() {
    ...
    try {
      const result = await api.allureGenerate();
      setStatus(result);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="rounded-lg border border-line bg-white/60 shadow-panel p-4 space-y-2">
      ...
      <p className="text-xs text-ink/60">Builds from every run recorded so far (allure-playwright records results automatically as scripts run).</p>
