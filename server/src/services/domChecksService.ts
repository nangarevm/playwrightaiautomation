...
// display:none/visibility:hidden/opacity:0 are excluded, since that's not a bug),
// overlapping interactive elements (two clickable targets whose boxes
// significantly intersect -- a click may land on the wrong one), text that
// clips instead of wrapping/ellipsizing (overflow:hidden with no ellipsis and
// content wider than the box), and interactive elements positioned entirely
// outside the horizontal viewport while still not marked aria-hidden (a mouse
// user can never reach them, but they may still be tab-focusable).
//
...
// Overlap detection is O(n^2) over interactive elements -- skip it on a page
// with an unusually large interactive-element count rather than hang the scan.
const MAX_ELEMENTS_FOR_OVERLAP_CHECK = 300;

export async function runDomChecks(page: Page): Promise<DomIssue[]> {
  const raw = await page
    .evaluate((maxForOverlap: number) => {
      ...
      // 3. Text overflow/truncation: content wider/taller than its box, overflow
      // hidden, and no ellipsis affordance to signal the truncation to the user.
      ...
      for (const el of allLeafEls) {
        ...
        if (hidesOverflow && clips && !hasEllipsis) {
      ...
      if (overflow.length > 0) {
        issues.push({
          kind: "text_overflow",
          severity: "low",
          message: `${overflow.length} element(s) clip their text (content wider than the box, overflow hidden, no ellipsis) -- likely truncated/unreadable text.`,
      ...
      for (const el of interactive) {
        if (offViewport.length >= 10) break;
      ...
      return issues;
    }, MAX_ELEMENTS_FOR_OVERLAP_CHECK)
