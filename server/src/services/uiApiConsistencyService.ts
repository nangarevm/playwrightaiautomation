...
export function createConsistencyRule(input: { screenId: string; name?: string; domSelector: string; apiEndpointKey: string; jsonPath?: string }): ConsistencyRuleRow {
  ...
  db.prepare(`
    INSERT INTO ui_api_consistency_rules (id, screen_id, name, dom_selector, api_endpoint_key, json_path, is_active, created_at)
    VALUES (@id, @screen_id, @name, @dom_selector, @api_endpoint_key, @json_path, @is_active, @created_at)
  `).run(row);
    ...
    if (uiCount !== apiCount) {
      findings.push(
        recordBugFinding({
          ...
          detail: `The page renders ${uiCount} element(s) matching "${rule.dom_selector}", but ${rule.api_endpoint_key}${rule.json_path ? ` at ${rule.json_path}` : ""} returned ${apiCount}.`,
          screenId,
          runId,
          evidence: { rule: { selector: rule.dom_selector, endpointKey: rule.api_endpoint_key, jsonPath: rule.json_path }, uiCount, apiCount },
          stepsToReproduce: [
            ...
            "Observe: the two counts disagree.",
          ],
        })
      );
    }
